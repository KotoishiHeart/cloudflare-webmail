...
provider header limits. The relationship is also useful operational metadata
...
limited to the newest 100 entries, and trimmed to a 2,048-byte custom-header
safety limit. Replies persist and send both `In-Reply-To` and
...
- Thread headers used by the R2 compose snapshot, D1 row, and SMTP2GO request
  come from one server-derived value.

...
| Archived D1/R2 migration | Rebuilt | Safe SQL is parsed into an isolated SQLite adapter, account mapping is explicit, a current provisioning manifest and external-policy review are generated, target mailbox/address/owner prerequisites are guarded, raw snapshots are resumable, MIME is rebuilt, bulk R2 comparison precedes D1, and final D1/R2 audit is read-only. |

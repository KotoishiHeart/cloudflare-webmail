...
| Domain DNS, Email Routing, and forwarding policy management | External | The deployment manifest names SMTP2GO sender and Email Routing domains. Preflight reads Cloudflare routing state; SMTP2GO sender verification and DNS alignment remain separately reviewed instead of being mutated from the mail application. |
...
| SMTP2GO provider API | Rebuilt | Jobs uses a new bounded HTTPS adapter with a fixed endpoint, header-only Worker Secret, provider-neutral Queue contract, attachment integrity checks, and explicit permanent/retryable error mapping. The primary mailbox address is the only permitted From identity. |
...
| Archived behavior | Rebuild status | Current implementation/evidence |
| --- | --- | --- |
| Labels and user display settings | Rebuilt | Labels are mailbox-scoped with composite foreign keys; preferences, including the authorized default mailbox, are keyed by the stable internal user ID. Archived global labels, assignments, per-user display preferences, and the global default From are deterministically converted or provisioned with provenance and authorization checks. |
...
| Archived D1/R2 migration | Rebuilt | Safe SQL is parsed into an isolated SQLite adapter, account mapping is explicit, a current provisioning manifest and external-policy review are generated, a hash-bound verifier checks Access/domain/directory consistency, target mailbox/address/owner prerequisites are guarded, a frozen raw-key list is fetched in bulk and verified resumably, MIME is rebuilt, bulk R2 comparison precedes D1, and final D1/R2 audit is read-only. |
...
| Public inbound simulation and direct MX SMTP probing | Excluded | Canary verification uses real Access identities, Email Routing, and SMTP2GO. There is no production test endpoint that can inject mail around the Ingest boundary. |
